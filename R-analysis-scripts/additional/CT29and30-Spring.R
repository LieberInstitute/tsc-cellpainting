...
AUC3 <- data.frame(matrix(ncol=3,nrow=11))
colnames(AUC3) <- c("ranger","glmnet","mlp")
...
for (i in seq_along(models)) {
  ...
  AUC3[,i] <- modelSex$resample$ROC
}

write.csv(AUC,file="caret_Sex_Normoxia_Spring.csv")

#Hypoxia
features_hypoxia <- features[features$Metadata_Oxygen == 'Hypoxia',]

AUC4 <- data.frame(matrix(ncol=3,nrow=11))
colnames(AUC4) <- c("ranger","glmnet","mlp")
...
for (i in seq_along(models)) {
  ...
  AUC4[,i] <- modelSex$resample$ROC
}

write.csv(AUC,file="caret_Sex_Hypoxia_Spring.csv")
