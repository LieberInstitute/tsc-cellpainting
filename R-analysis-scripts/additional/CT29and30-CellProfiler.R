...
AUC <- data.frame(matrix(ncol=3,nrow=11))
...
write.csv(AUC,file="caret_Sex_Normoxia_features.csv")

#Hypoxia
features_hypoxia <- features[features$Metadata_Oxygen == 'Hypoxia',]

AUC2 <- data.frame(matrix(ncol=3,nrow=11))
colnames(AUC2) <- c("ranger","glmnet","mlp")
...
for (i in seq_along(models)) {
  ...
  AUC2[,i] <- modelSex$resample$ROC
}

write.csv(AUC,file="caret_Sex_Hypoxia_features.csv")
